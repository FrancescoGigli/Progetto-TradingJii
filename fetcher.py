...
async def fetch_data_for_multiple_symbols(exchange, symbols, timeframes, limit=1000):
    """
    Recupera i dati per più simboli e timeframe contemporaneamente.
    ...
    Returns:
        dict: Dizionario con i dati per ogni combinazione simbolo/timeframe
    """
    results = {}
    tasks = []
    
    # Crea una task per ogni combinazione simbolo/timeframe
    for symbol in symbols:
        results[symbol] = {}
        for timeframe in timeframes:
            task = fetch_and_save_data(exchange, symbol, timeframe, limit)
            tasks.append((symbol, timeframe, asyncio.create_task(task)))
    
    # Esegui tutte le task in parallelo e attendi i risultati
    for symbol, timeframe, task in tasks:
        try:
            ...
            if df is not None:
                ...
                logging.warning(f"Nessun dato disponibile per {symbol} ({timeframe})")
        except Exception as e:
            logging.error(f"Errore nel recupero dati per {symbol} ({timeframe}): {e}")
