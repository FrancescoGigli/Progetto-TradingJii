...
class ModelTrainer:
    ...
    def pattern_to_signal(self, pattern: str, num_samples: int) -> List[int]:
        ...
        Args:
            pattern: Pattern string from categorize_series
            num_samples: Number of samples to create labels for
            
        Returns:
            List of signal labels (1=BUY, 0=HOLD, -1=SELL)
        """
        # Map patterns to trading signals based on volatility behavior
        # This is a simple heuristic - you can improve this based on domain knowledge
        
        pattern_mapping = {
            # Rising volatility patterns -> potential breakout -> BUY
            "1111111": 1,  # Consistently rising
            "0011111": 1,  # Rising trend
            "0001111": 1,  # Accelerating up
            
            # Falling volatility patterns -> potential breakdown -> SELL  
            "0000000": -1,  # Consistently falling
            "1100000": -1,  # Falling trend
            "1110000": -1,  # Accelerating down
            
            # Mixed/neutral patterns -> HOLD
            "0101010": 0,   # Oscillating
            "1010101": 0,   # Oscillating
            "0110110": 0,   # Mixed pattern
        }
        
        # Default signal based on pattern characteristics
        if pattern in pattern_mapping:
            signal = pattern_mapping[pattern]
        else:
            # Heuristic: if more 1s than 0s -> bullish (BUY), otherwise neutral (HOLD)
            ones_count = pattern.count('1')
            zeros_count = pattern.count('0')
            
            if ones_count > zeros_count * 1.5:  # Significantly more rising periods
                signal = 1  # BUY
            elif zeros_count > ones_count * 1.5:  # Significantly more falling periods  
                signal = -1  # SELL
            else:
                signal = 0  # HOLD
